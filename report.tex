...
\subsection{Puller担当}
私は、MVCモデルの中核となるModel層の設計と実装を主導しました。ゲームの根幹となるメインループ、オブジェクト指向設計に基づくクラス階層の構築、物理演算（当たり判定）、および敵AIのロジックを担当しました。

\subsubsection{堅牢なオブジェクト管理システム}
ゲーム内に登場するプレイヤー、敵、弾丸などの全オブジェクトを効率的に処理するため、基底クラス`GameObject`を作成し、`ArrayList`で一元管理する構造を採用しました。
特筆すべき点として、メインループ(`update`)内でオブジェクトリストを走査中に新たなオブジェクト（発射された弾丸など）を追加すると`ConcurrentModificationException`が発生する問題を防ぐため、一時的なバッファリスト`newObjectsBuffer`を導入しました。これにより、ループ終了後に安全にリストを統合する堅牢な設計を実現しました。

\begin{lstlisting}[caption={安全なオブジェクト更新ロジック (GameModel.java)}]
// ループ中のエラー回避用のバッファ
private ArrayList<GameObject> newObjectsBuffer;

public void update() {
    // ... (省略) ...
    
    // スポナーや入力処理によって生成されたオブジェクトをバッファに追加
    
    // 安全なタイミングでメインリストに統合
    objects.addAll(newObjectsBuffer);
    newObjectsBuffer.clear();

    // 全オブジェクトの移動と描画を一括処理（ポリモーフィズム）
...
\subsubsection{精密な当たり判定と領域計算}
シューティングゲームにおいて最も重要な当たり判定には、単純な矩形交差判定（`intersects`）だけでなく、`java.awt.geom.Area`クラスを用いた領域計算アルゴリズムを採用しました。
単純な四角形同士の判定では、画像が透明な部分でも「当たった」と判定されてしまう問題があります。そこで、`Area`クラスの`intersect`メソッドを使用することで、オブジェクトの形状に基づいた論理積を計算し、ピクセル単位に近い精密な衝突判定を実現しました。

\begin{lstlisting}[caption={Areaクラスを用いた精密判定 (GameModel.java)}]
private boolean checkIntersection(GameObject obj1, GameObject obj2) {
    // 1. 形状（Shape）を取得
    Shape s1 = obj1.getShape();
    Shape s2 = obj2.getShape();

    // 2. 高速化のため、まずは境界矩形で簡易判定
    if (!s1.getBounds2D().intersects(s2.getBounds2D())) {
        return false;
    }

    // 3. Areaクラスによる精密判定（形状の重なりを計算）
    Area area1 = new Area(s1);
    Area area2 = new Area(s2);
    area1.intersect(area2);

    return !area1.isEmpty(); // 重なり部分が存在すれば衝突
}
\end{lstlisting}

\subsubsection{弾丸の優先度システム (Power Level)}
単なる「当たったら消える」だけでなく、弾丸に「強度（Power Level）」の概念を導入しました。
`Projectile`クラスに`powerLevel`フィールドを持たせ、弾丸同士が衝突した際、強度の高い方が生き残るロジックを実装しました（例：岩(Lv2)は矢(Lv1)を破壊して貫通するが、太陽(Lv3)には破壊される）。これにより、戦略的な攻撃の選択が可能になりました。

\begin{lstlisting}[caption={弾丸同士の相殺ロジック (GameModel.java)}]
if (p1.getPowerLevel() > p2.getPowerLevel()) {
    p2.setDead(); // p1が強く、p2が破壊される
} else if (p2.getPowerLevel() > p1.getPowerLevel()) {
    p1.setDead(); // p2が強く、p1が破壊される
} else {
    p1.setDead(); // 同等の場合は相殺
    p2.setDead();
}
...
\subsubsection{ジェネリクスを用いた敵生成}
敵の出現管理を行う`EnemySpawner`クラスでは、Javaのジェネリクス（`Class<? extends Minion>`）を活用しました。これにより、スパナーのインスタンス生成時に「ハーピー」や「サイクロプス」といったクラスタイプを動的に指定するだけで、異なる種類の敵を生成可能にしました。また、出現間隔に乱数による「ゆらぎ」を持たせることで、単調なゲームプレイを防いでいます。

\subsubsection{第一ボス (Apollo) のAI実装}
ステージ1のボスである`Apollo`クラスを実装しました。継承元の`Boss`クラスの機能を活用しつつ、画面端での反射移動や、HP半減時に画像と移動速度が変化する「発狂モード（第二形態）」への状態遷移ロジックを組み込みました。

{\raggedleft (文責：Puller)\par}
...
\subsection{達成度と設計の評価}
本プロジェクトの当初の目標は、JavaのSwingライブラリを用いて、ステージ進行やボス戦を含む本格的な縦スクロールシューティングゲームを作成することであった。完成したプログラムは、複数の敵タイプ、段階的な難易度上昇、ボスの形態変化、およびパワーアップシステムを備えており、当初の仕様を十分に満たすものが完成したと評価できる。

設計段階でMVCアーキテクチャを採用したことは、開発の効率化に大きく寄与した。Model（論理）とView（描画）を分離したことで、ロジックの修正とUIの調整を並行して行うことが可能となり、チーム開発における作業効率を大幅に向上させることができた。

\subsection{技術的課題：当たり判定の最適化}
技術的な課題として、当たり判定の計算コストが挙げられる。現在の実装では、画像の透明部分を考慮したAreaクラスによる領域計算を採用しており、精密な判定を実現している。しかし、全てのオブジェクト同士を総当たりで判定しているため、計算量は $O(N^2)$ となる。弾幕ゲームのようにオブジェクト数が膨大になる場合、この方式では処理落ちが発生する可能性があるため、今後は「四分木（Quadtree）」などの空間分割手法を導入し、判定回数を削減する最適化が必要である。

\subsection{今後の展望と機能拡張}
本プログラムをより魅力的なゲームへと発展させるため、以下の機能拡張を計画している。

\begin{enumerate}
    \item \textbf{ステージ3の完結とコンテンツ拡充}:
    現在実装中のステージ3において、新たな敵キャラクターとして「ミノタウロス」を追加し、道中のバリエーションを強化する。また、ステージの最後には最終ボスとして「ハデス（Hades）」を登場させ、物語のクライマックスにふさわしい高難易度の戦闘を実装する予定である。

    \item \textbf{無限エキストラステージの実装}:
    最終ボス撃破後、難易度が動的に上昇し続ける「無限エキストラステージ」を設ける。これにより、プレイヤーが自身の限界に挑戦し、ハイスコアを競い合えるようなリプレイ性の高い仕組みを導入したい。

    \item \textbf{音響効果（BGM・SE）の導入}:
    現在は視覚情報のみの構成となっているが、ゲームの没入感を高めるために、Java Sound API等を用いた音楽の再生機能を実装する。ステージごとのBGMや、攻撃・被弾時の効果音（SE）を加えることで、より臨場感のあるプレイ体験の提供を目指す。
\end{enumerate}

以上の改良と拡張を行うことで、より完成度の高い、エンターテインメント性に優れたゲームエンジンへと発展させることが可能であると考える。
...
\subsection{Puller}
本演習を通して、Javaによるオブジェクト指向プログラミングとMVCアーキテクチャの実践的な理解を深めることができました。特に、Model層の実装において「継承」と「ポリモーフィズム」を活用することで、数百個のオブジェクト（敵や弾丸）をわずか数行のループで一括管理できる拡張性の高さに感動しました。

グループワークの側面では、Git/GitHubを用いたバージョン管理の重要性を痛感しました。開発終盤において、各メンバーの変更（UIの調整やロジックの追加）を統合（マージ）する際にコンフリクトが発生することもありましたが、それを解消する過程で、他者のコードを読み解く力や、変更の影響範囲を予測する力が養われたと感じています。
