...
\subsection{齋藤担当}
私が作成したのは、Zeus.java、Lighting.java、GamePanel.javaの一部、GameConstants.javaの一部です。

まず、GamePanel.javaについての説明です。ここで、担当した部分は、382行目から505行目までです。ここは、コントローラーの部分でキー操作を行なっています。
...
続いて、16行目から47行目です。ゲームをしている時は、Playerの移動だけではなく、矢を放ったり、一時停止や、特殊能力まであり、ここもこれら全て条件分岐で処理をしています。まず、移動は、キーが押されたときの処理を行っているため、押されている方向キーのところは全てtrueに変更しています。そして、方向キーの押されているかの処理を終えた後、Playerが動かないといけないので、updatePlayerVelocityを実行し、方向キーの変更を移動という形で表せるようにした。他の動作については、動作にあったキーを押された時に、その動作の関数を呼び出します。

以降、他の状態については、その状態に操作できるキーが押されて時は、その動作ができる関数を呼び出して処理できるようにしています。
...
この関数は、keyPressedで行われた変更を元の状態に戻します。このようにすることで、毎フレーム押された状態だけを処理することができます。
